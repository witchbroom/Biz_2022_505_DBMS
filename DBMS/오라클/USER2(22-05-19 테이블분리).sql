...
/*
성적데이터가 저장된 tbl_score table의 데이터 중
...
-- projection : 컬럼을 원하는 것만, 원하는 순서로 보여달라
...
SELECT sc_seq, sc_stnum, 
 (
    SELECT sb_name FROM tbl_subject
...
-- 2. sub query를 사용하여
-- tbl_subject talbe 로부터 과목코드를 가져와서
...
ALTER TABLE tbl_score MODIFY (sc_subcode NOT NULL);
-- 이후 부터 데이터를 INSERT 할때 sc_sbcode 칼럼의 데이터가 없으면 INSERT가 거부된다
