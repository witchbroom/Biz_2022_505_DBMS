...
INSERT INTO tbl_dept(d_code, d_name, d_pro, d_class) VALUES ('D0003', '법학과', '장영자', '505');
...
/*
테이블에 일련번호 칼럼이 있을 경우
중복되지 않은 일렵련번호를 사용하여 PK로사용한다
오라클 이외의 다른 DBMS에서는 AUTO INCREMENT 라는 속성을 사용하여
데이터를 INSERT 할때 자동을 칼럼 데이터를 추가한다
하지만 오라클은 그러한 속성이 없어서 상당히 불편하다

오라클에서는 SEQUENTE 라는 특별한 개체가 있어서
그 객체를 사용하여 같은 효과를 발휘한다

또는 pk 해당하는 칼럼을 문자열 32byte 크기로 설정하고
UUID 를 사용하기도 한다
프로그래밍 언어(Java, c, phthon : 호스트 언어)와 함께 프로젝트를 할때는 UUID를 사용한다
*/

-- seq_belong 이라는 SEQUENCE 객체를 생성하고
-- 시작값을 1로 자동증가 옵션을 1로 설정하겠따
...
-- 그래서 의미없는(DUMMY) Table인 DUAL Table을 사용하여 FROM 절을 붙여준다
SELECT 10 + 20 FROM DUAL;
SELECT 100 * 100 FROM DUAL;
SELECT 100 - 10 FROM DUAL;
SELECT 50 / 20 FROM DUAL;

-- SEQUENCE 를 사용하여 자동 증가하는 SEQ 를 만들고 INSERT 해서 사용하기
...
-- tbl_student 에 없는 202200004 학번을 추가하려고 했더니
...
-- child key found
-- FK 를 설정할때
-- parent table 의 데이터를 삭제하려고 할 때 child table 에 데이터 있으면
-- 보통 삭제 금지,
-- parent table 데이터를 변경(Update) 하려고 할 때 child table 에 데이터 있으면
-- 보통 변경 금지
