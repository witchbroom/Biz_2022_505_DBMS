...
*/
SELECT sc_stnum, SUM(sc_score), ROUND(AVG(sc_score),2)
FROM tbl_score
GROUP BY sc_stnum
ORDER BY ROUND(AVG(sc_score),2) DESC;
...
/*
통계함수와 GROUP BY 를 사용하여 통계연산을 수행할 때 주의사항
통계연산 결과에 어떤 조건을 부여하여 데이터를 보고자 할때는
WHERE 절이나 HAVING 절에 조건을 부여할 수 있다

통계연산이 수행된 결과에 대하여 조건을 부여할 때는
어쩔수없이 HAVING 절에 조건을 부여해야 할 것이다
